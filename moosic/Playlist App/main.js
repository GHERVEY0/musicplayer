...
function update_second() {
    ...
    if(!isNaN(song[index_no].duration)) {
        ...
        slider.value = position; // update slider position!11!1!1
    }
    ...
    total_duration.textContent = `${durationMinutes}:${durationSeconds < 10 ? '0' : ''}${duration_seconds}`;
...
down_player.addEventListener('click', function(){
    p_m_player.style.transofmr = 'trastlateY(110%)'; //hide popup player
});
...
forward_btn.addEventListener('click', function() {
    index_no = (index_no + 1) % All_song.length; //move to new song
